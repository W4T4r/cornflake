  ...
  outputs = {
    ...
    let
      ...
      packageCatalogFor = system:
        builtins.foldl' (acc: attrs: acc // attrs) { } [
          ((if bcompare5Flake ? packages && builtins.hasAttr system bcompare5Flake.packages then {
            bcompare5 = bcompare5Flake.packages.${system}.default;
          } else { }))
          ((if catppuccinFcitx5 ? packages && builtins.hasAttr system catppuccinFcitx5.packages then {
          ...
          ((if catppuccinGtk ? packages && builtins.hasAttr system catppuccinGtk.packages then {
            catppuccin-gtk = catppuccinGtk.packages.${system}.default;
          } else { }))
          ((if catppuccinKvantum ? packages && builtins.hasAttr system catppuccinKvantum.packages then {
    ...
    in
    {
      ...
      apps = forAllSystems (system:
        if bcompare5Flake ? apps && builtins.hasAttr system bcompare5Flake.apps then {
